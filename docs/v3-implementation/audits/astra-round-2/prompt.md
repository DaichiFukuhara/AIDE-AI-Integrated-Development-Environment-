...
起草担当とは別のCodex CLI / gpt-6-astraの監査として、添付全文を直接照合してください。
ユーザーは監査済みv3設計を「実装して」と依頼しました。設計A2に従いMarkdown規約・役割・テンプレートとして実装し、入力固定の補助Pythonと小さな製品実行例を添えています。
製品CLI/UIや自動採用エンジンは設計の前提ではありません。ただし手順が実行可能な具体性を持つか、実装として契約に不足・矛盾がないかは厳密に確認してください。

ツール・シェル・外部検索・編集・別エージェントは使わず、後半の元行番号付きファイル全文を読んでください。過去の設計passや今回の自己点検は正しさの根拠にしないでください。hash値は提示された識別情報で、あなた自身の再計算として扱わないでください。

基準は添付の監査済み設計（tree 80のG-V3と各system）、および実装criteriaのDDD/AIDE観点です。
確認項目: 元の4階層・DDD正本、必要な枝からの計画開始、実装・テスト・実使用・採否・設計反映、daily-passと初回/境界/周期監査、終了通知、取消・再送・中断・競合、subjectの版固定（実装・証拠・定義と依拠先）、影響scope、一括反映、独立監査と許可、テンプレートで保存できる項目、実行例の限界表示。補助ツールの不変性・パス処理・エラー処理とテストの十分性も確認してください。

保証phaseをplan/implementationとし、adoption/periodicをimplementation phaseにした点は内部表現の具体化です。実装担当は意味を変えたつもりはありませんが、設計との不一致があれば具体的反例で指摘してください。
演習はローカルの製品テスト・修正・サンプルCLI操作・証拠固定を実行するもので、独立監査や採用を実行済みとは表示していません。自動化していないことと契約未実装を区別してください。

回答は日本語Markdown、判定pass/fail/blocked。open major/blockerがあればfail。
指摘は安定ID、severity、設計条件/基準、ファイル行、具体的反例、影響、最小の解消条件。
読んだ対象、未読・限界、3systemの実装充足、テストの判定と運用効果未実証の区別を明記。
好みの機能追加、特定技術の強制、一般的な将来不安だけをmajorにせず、実際の不足は見逃さないでください。

前回の実装監査報告も添付します。AIV3-001（費用不明時の停止）、002（保存中の障害検証）、003（古いPythonでのjunction検出）の解消と回帰を確認し、各指摘をclosed/openで明記してください。最新検証はverification/round-2、費用の手動シナリオはverification/budget-scenarios.mdです。以前の9テスト記録は履歴で、最新は12テストです。
...
<file path="docs/v3-implementation/README.md" sha256="359b05e425ed8061d8bd380494f1fe637065e3207ad0bfdb67bc6043eac08712">
0001: # AIDE v3の実装
0002: 
0003: [harness-v3](../../harness-v3/README.md)に、監査済み設計tree 80をMarkdownハーネスとして実装した。
0004: この実装は実行規約、役割手順、テンプレート、入力固定ツール、実行例で構成する。独立Astraの初回監査はmajor 1件・minor 2件でfail。3件を修正し再監査する。
0005: 
0006: ## 設計との対応
0007: 
0008: | 設計条件 | 実装先 | 確認方法 |
...
0022: ## 検証
0023: 
0024: - [補助ツール検証](verification/round-2/snapshot-tests.json): 不変保存、再実行、改変・欠落・不正manifest、パス逸脱、保存中の書込み・公開rename失敗、内部junctionを確認。
