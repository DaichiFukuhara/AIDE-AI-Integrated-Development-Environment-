...
<file path="docs/v3-implementation/README.md" sha256="e2dd4319a6ef6a0db588b9eea0f068c360e2d09f9b20c4504f6d4a5990aa27f2">
0001: # AIDE v3の実装
0002: 
0003: [harness-v3](../../harness-v3/README.md)に、監査済み設計tree 80をMarkdownハーネスとして実装した。
0004: この実装は実行規約、役割手順、テンプレート、入力固定ツール、実行例で構成する。独立Astraによる実装監査は実施待ち。
0005: 
0006: ## 設計との対応
0007: 
0008: | 設計条件 | 実装先 | 確認方法 |
...
0022: ## 検証
0023: 
0024: - [補助ツール検証](verification/snapshot-tests.json): 不変保存、再実行、改変・欠落・不正manifest、パス逸脱、途中失敗を確認。
0025: - [実行例の結果](verification/exercise.json): 意図的な件数計算の不具合を同じテストで検出し、修正版5テストとサンプルCLI操作が成功。旧試行の内容を保持。
0026: - 最初の通常サンドボックスでの実行は、Windowsの一時ディレクトリへのアクセス拒否で開始できなかった。同じ検証を通常のローカル権限で実行し、個別の一時フォルダと保存結果だけを作成した。ネットワークは使わない。
0027: 
0028: snapshotツールは選択したファイルの内容固定と改変検出だけを行う。意味分類、台帳の更新、許可・監査判断、採用はMarkdown手順を実行する担当の責任である。
0029: 実行例はサンプルデータの自動演習であり、独立監査・全採用手順・実ユーザー評価を実施済みと扱わない。
0030: 
0031: ## 設計・履歴の境界
0032: 
0033: [元の設計](../v3-design/README.md)の20文書と既存閉包・監査原文は変更しない。v2からの移行は必要な枝だけを参照付きで行う。
0034: v3を使った継続的な開発で、最初の実物までの時間、監査停止回数、手戻り、記録不一致、本人理解を今後測る。
</file>
...
<file path="harness-v3/roles/experiment.md" sha256="d2f2c138e589d8c1e82814be6ecd424d9f9efbf1da38a938a2374fceb2e15b32">
...
0014: 
0015: 中断後は対象版・委任・残予算を再確認する。同じ実行要求の再送で新trialを勝手に増やさない。
0016: 上限に達したら結果と次の案を返す。pausedは再開条件を持ち、終了とは区別する。
0017: 確率的な結果は計画の分布・許容誤差等の範囲にだけ結論を限定する。
</file>
...
<file path="harness-v3/roles/orchestrate.md" sha256="05b20d87cce1197d8a3ec8c7019c20d3bf8ff8d583f4919bd55f8e5bbd0a24e4">
...
0010: 6. checkedの計画はexperimentへ、評価済みの採用候補はrecordへ、構造不足はdesignへ渡す。
...
<file path="harness-v3/templates/experiment.md" sha256="62693daff8f4e222080952057568abea1fb7e459338ac260e00ef66626935532">
0001: ---
...
0023: ## 仮説と最小出力
0024: 
0025: <何を作り、何が分かれば次の判断ができるか>
0026: 
0027: ## 範囲と上限
0028: 
0029: <src/testsの対象、除外範囲、担当、予算、終了条件、許可>
0030: 
0031: ## 固定した評価条件
0032: 
0033: <条件ID、回帰・結合・仮説比較・実使用・人の評価、失敗と未実行の扱い。
0034: 確率的出力では分布・誤差・反復数・データ等も計画する>
0035: 
0036: ## 採否と反映
0037: 
0038: <採用/修正/不採用/保留、理由、adoption IDと確定bundle>
0039: 
0040: ## 中断・終了・再開
0041: 
0042: <残予算、再開条件、inconclusiveなら継続か終了か。
0043: 終端とcycle_closed outboxを同時に保存し、全関連操作の確定結果を添える>
</file>
...
<file path="harness-v3/templates/trial.md" sha256="9a92312afeef600c4f90794ed8f8b1d73a5a63175b40202ab20d260302c4cd23">
0001: ---
...
0016: ## 実行条件と生の結果
0017: 
0018: <コマンド、環境、データ、終了コード、標準出力・エラーの不変参照。
0019: テストpass/fail/not-run/N-Aを理由付きで区別する>
0020: 
0021: ## 実使用と人の評価
0022: 
0023: <実際に操作した結果、観測者、本人確認の有無。模擬データと実ユーザーを区別する>
0024: 
0025: ## 評価・限界・次の案
0026: 
0027: <計画条件ごとの成立、比較不能、版不一致、採否の推奨。保証を別条件へ一般化しない>
0028: 
0029: この記録を訂正・再実行するときは新trial_idを発行し、旧版への参照を残す。
</file>

<file path="harness-v3/tests/test_snapshot.py" sha256="e483b70fc004bc56a79dfaeaef904a9bd220752a963cb34386100dd0d7fd87e6">
...
0008: 
0009: TOOL = Path(__file__).resolve().parents[1] / "tools/snapshot.py"
0010: spec = importlib.util.spec_from_file_location("snapshot", TOOL)
0011: snapshot = importlib.util.module_from_spec(spec)
0012: spec.loader.exec_module(snapshot)
0013: 
0014: 
0015: class SnapshotTests(unittest.TestCase):
0016:     def setUp(self):
0017:         self.tmp = tempfile.TemporaryDirectory()
0018:         self.addCleanup(self.tmp.cleanup)
0019:         self.root = Path(self.tmp.name)
0020:         (self.root / "design").mkdir()
0021:         (self.root / "design/master.md").write_text("# 目標\n最小の動作\n", encoding="utf-8")
0022:         (self.root / "app.py").write_bytes(b"print('first')\r\n")
0023: 
0024:     def freeze(self):
0025:         return snapshot.freeze(self.root, ["design/master.md", "app.py"])
0026: 
0027:     def test_deterministic_and_byte_preserving(self):
0028:         a = self.freeze()
0029:         b = snapshot.freeze(self.root, ["app.py", "design/master.md"])
0030:         self.assertEqual(a, b)
0031:         self.assertEqual((a / "files/app.py").read_bytes(), b"print('first')\r\n")
0032:         self.assertEqual(snapshot.verify(a)["snapshot_id"], a.name)
0033: 
0034:     def test_new_input_does_not_rewrite_old_evidence(self):
0035:         old = self.freeze()
0036:         (self.root / "app.py").write_text("print('second')\n")
0037:         new = self.freeze()
0038:         self.assertNotEqual(old, new)
0039:         self.assertEqual((old / "files/app.py").read_bytes(), b"print('first')\r\n")
0040:         snapshot.verify(old)
0041: 
0042:     def test_tampered_or_missing_evidence_fails(self):
0043:         frozen = self.freeze()
0044:         (frozen / "files/app.py").write_bytes(b"changed")
0045:         with self.assertRaises(snapshot.SnapshotError):
0046:             snapshot.verify(frozen)
0047:         with self.assertRaises(snapshot.SnapshotError):
0048:             self.freeze()  # Existing damaged snapshot must never be silently repaired.
0049:         (frozen / "files/app.py").unlink()
0050:         with self.assertRaises(snapshot.SnapshotError):
0051:             snapshot.verify(frozen)
0052: 
0053:     def test_manifest_corruption_fails(self):
0054:         frozen = self.freeze()
0055:         p = frozen / "manifest.json"
0056:         data = json.loads(p.read_text())
0057:         data["files"][0]["path"] = "../escape"
0058:         p.write_text(json.dumps(data))
0059:         with self.assertRaises(snapshot.SnapshotError):
0060:             snapshot.verify(frozen)
0061: 
0062:     def test_path_escape_duplicate_and_empty_inputs_fail(self):
0063:         for paths in (["../outside"], [str(self.root / "app.py")],
0064:                       ["app.py", "app.py"], []):
0065:             with self.subTest(paths=paths), self.assertRaises(snapshot.SnapshotError):
0066:                 snapshot.freeze(self.root, paths)
0067:         self.assertFalse((self.root / "design/snapshots").exists())
0068: 
0069:     def test_failure_does_not_publish_partial_snapshot(self):
0070:         with self.assertRaises(snapshot.SnapshotError):
0071:             snapshot.freeze(self.root, ["app.py", "missing.py"])
0072:         self.assertFalse((self.root / "design/snapshots").exists())
0073: 
0074:     def test_symbolic_link_is_rejected_when_available(self):
0075:         try:
0076:             (self.root / "link.py").symlink_to(self.root / "app.py")
0077:         except OSError:
0078:             self.skipTest("Creating symlinks is unavailable on this host")
0079:         with self.assertRaises(snapshot.SnapshotError):
0080:             snapshot.freeze(self.root, ["link.py"])
0081: 
0082:     def test_self_consistent_manifest_cannot_read_outside_bundle(self):
0083:         body = {"format_version": 1, "files": [
0084:             {"path": "../../app.py", "size": 16, "sha256": "0" * 64}
0085:         ]}
0086:         identity = snapshot.digest(snapshot.canonical(body))
0087:         folder = self.root / ("SN-" + identity)
0088:         folder.mkdir()
0089:         manifest = dict(body, snapshot_id=folder.name, digest=identity)
0090:         (folder / "manifest.json").write_bytes(snapshot.canonical(manifest))
0091:         with self.assertRaisesRegex(snapshot.SnapshotError, "Project-relative"):
0092:             snapshot.verify(folder)
0093: 
0094:     def test_cli_returns_failure_for_bad_input(self):
0095:         p = subprocess.run([sys.executable, str(TOOL), "freeze", str(self.root), "missing"],
0096:                            capture_output=True, text=True)
0097:         self.assertEqual(p.returncode, 1)
0098:         self.assertIn("snapshot:", p.stderr)
0099: 
0100: 
...
<file path="harness-v3/tools/README.md" sha256="02eb064909048014ce284beb6424f201d5013faad918c8af0fb32926f14cca80">
0001: # 監査入力の保存を補助する
0002: 
...
0020: 
0021: 保存中は指定元ファイルへの並行書込みを停止する。一つのsnapshot内の全入力が同じ論理版であることは担当が開始前と保存後に照合する。
0022: このツールは各ファイルの固定・検出を行うが、編集者とのロックや意味版判定を代行しない。
0023: 通常の状態追記で既存subjectを再保存する必要はない。固定した意味入力を維持し、状態・結果を別記録へ置く。
</file>

<file path="harness-v3/tools/snapshot.py" sha256="ffb3e81d2698b096c878294cbd1639fef10d47c91a6a1b4711aac85d64a24ca7">
...
0010: 
0011: 
0012: class SnapshotError(ValueError):
0013:     pass
0014: 
0015: 
0016: def canonical(value):
0017:     return json.dumps(value, sort_keys=True, ensure_ascii=False,
0018:                       separators=(",", ":"), allow_nan=False).encode("utf-8")
0019: 
0020: 
0021: def digest(data):
0022:     return hashlib.sha256(data).hexdigest()
0023: 
0024: 
0025: def checked_path(root, relative):
0026:     """Reject traversal and links before reading or writing an input path."""
0027:     relative = Path(relative)
0028:     if relative.is_absolute() or not relative.parts or ".." in relative.parts:
0029:         raise SnapshotError(f"Project-relative path required: {relative}")
0030:     candidate = root / relative
0031:     current = root
0032:     for part in relative.parts:
0033:         current = current / part
0034:         if current.is_symlink() or getattr(current, "is_junction", lambda: False)():
0035:             raise SnapshotError(f"Link is not an immutable input path: {current}")
0036:     if not candidate.resolve().is_relative_to(root):
0037:         raise SnapshotError(f"Path outside project: {relative}")
0038:     return candidate
0039: 
0040: 
0041: def verify(folder):
0042:     folder = Path(folder).resolve()
0043:     manifest_path = checked_path(folder, "manifest.json")
0044:     manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
0045:     if set(manifest) != {"format_version", "snapshot_id", "digest", "files"}:
0046:         raise SnapshotError("Unexpected manifest fields")
0047:     body = {key: manifest[key] for key in ("format_version", "files")}
0048:     expected = digest(canonical(body))
0049:     if (manifest["format_version"] != 1 or manifest["digest"] != expected
0050:             or manifest["snapshot_id"] != "SN-" + expected
0051:             or folder.name != manifest["snapshot_id"]):
0052:         raise SnapshotError("Manifest identity mismatch")
0053:     if not isinstance(manifest["files"], list) or not manifest["files"]:
0054:         raise SnapshotError("Empty input bundle")
0055:     seen = set()
0056:     for entry in manifest["files"]:
0057:         if set(entry) != {"path", "sha256", "size"}:
0058:             raise SnapshotError("Unexpected file fields")
0059:         relative = entry["path"]
0060:         if not isinstance(relative, str) or relative.casefold() in seen:
0061:             raise SnapshotError("Duplicate or invalid file path")
0062:         seen.add(relative.casefold())
0063:         # Validate relative independently; prefixing an absolute path would hide it.
0064:         checked_path(folder, relative)
0065:         file = checked_path(folder, Path("files") / relative)
0066:         if not file.is_file():
0067:             raise SnapshotError(f"Missing input: {relative}")
0068:         data = file.read_bytes()
0069:         if len(data) != entry["size"] or digest(data) != entry["sha256"]:
0070:             raise SnapshotError(f"Input changed: {relative}")
0071:     return manifest
0072: 
0073: 
0074: def freeze(project, paths):
0075:     project = Path(project).resolve()
0076:     if not project.is_dir():
0077:         raise SnapshotError("Project does not exist")
0078:     container = checked_path(project, "design/snapshots")
0079:     inputs, seen = [], set()
0080:     for relative in paths:
0081:         file = checked_path(project, relative)
0082:         name = file.relative_to(project).as_posix()
0083:         if name.casefold() in seen:
0084:             raise SnapshotError(f"Duplicate input: {name}")
0085:         seen.add(name.casefold())
0086:         if not file.is_file() or file.resolve().is_relative_to(container.resolve()):
0087:             raise SnapshotError(f"Expected a regular source file: {name}")
0088:         data = file.read_bytes()
0089:         inputs.append((name, data))
0090:     if not inputs:
0091:         raise SnapshotError("At least one input is required")
0092:     inputs.sort(key=lambda item: item[0])
0093:     body = {"format_version": 1, "files": [
0094:         {"path": name, "sha256": digest(data), "size": len(data)}
0095:         for name, data in inputs
0096:     ]}
0097:     identity = digest(canonical(body))
0098:     manifest = dict(body, snapshot_id="SN-" + identity, digest=identity)
0099:     destination = container / manifest["snapshot_id"]
0100:     checked_path(project, destination.relative_to(project))
0101:     if destination.exists():
0102:         if verify(destination) != manifest:
0103:             raise SnapshotError("Existing immutable snapshot differs")
0104:         return destination
0105:     container.mkdir(parents=True, exist_ok=True)
0106:     # Only our private staging directory is cleaned up. Existing bundles are never rewritten.
0107:     with tempfile.TemporaryDirectory(prefix=".pending-", dir=container) as temporary:
0108:         staged = Path(temporary) / manifest["snapshot_id"]
0109:         staged.mkdir()
0110:         for name, data in inputs:
0111:             target = staged / "files" / name
0112:             target.parent.mkdir(parents=True, exist_ok=True)
0113:             target.write_bytes(data)
0114:         (staged / "manifest.json").write_bytes(canonical(manifest) + b"\n")
0115:         verify(staged)
0116:         try:
0117:             os.rename(staged, destination)
0118:         except OSError:
0119:             # An identical concurrent freeze may have published this immutable bundle.
0120:             if not destination.exists() or verify(destination) != manifest:
0121:                 raise
0122:     return destination
0123: 
0124: 
0125: def main(argv=None):
0126:     parser = argparse.ArgumentParser(description=__doc__)
0127:     commands = parser.add_subparsers(dest="command", required=True)
0128:     create = commands.add_parser("freeze", help="Freeze explicitly selected project files")
0129:     create.add_argument("project", type=Path)
0130:     create.add_argument("files", nargs="+")
0131:     check = commands.add_parser("verify", help="Check a saved bundle without changing it")
0132:     check.add_argument("snapshot", type=Path)
0133:     hash_json = commands.add_parser("hash-json", help="Canonical SHA-256 of a JSON input object")
0134:     hash_json.add_argument("input", type=Path)
0135:     args = parser.parse_args(argv)
0136:     try:
0137:         if args.command == "freeze":
0138:             result = {"snapshot": str(freeze(args.project, args.files))}
0139:         elif args.command == "verify":
0140:             result = {"verified": verify(args.snapshot)["snapshot_id"]}
0141:         else:
0142:             obj = json.loads(args.input.read_text(encoding="utf-8"))
0143:             if not isinstance(obj, dict):
0144:                 raise SnapshotError("Expected a JSON object")
0145:             result = {"sha256": digest(canonical(obj))}
0146:     except (OSError, ValueError, KeyError, TypeError) as error:
0147:         parser.exit(1, f"snapshot: {error}\n")
0148:     print(json.dumps(result, ensure_ascii=False))
0149:     return 0
0150: 
0151: 
0152: if __name__ == "__main__":
0153:     raise SystemExit(main())
</file>
